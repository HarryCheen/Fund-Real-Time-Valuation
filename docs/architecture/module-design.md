...
| `routes/holidays.py` | 节假日 API | `router` |
| `routes/websocket.py` | WebSocket 实时推送 | `router`, `ConnectionManager` |
| `routes/cache.py` | 缓存管理 API | `router` |
| `routes/datasource.py` | 数据源管理 API | `router` |
| `routes/overview.py` | 概览数据 API | `router` |
...
| 模块 | 文件 | 说明 |
|------|------|------|
| 数据网关 | `gateway.py` | 统一数据网关，支持故障切换和负载均衡 |
| 热备份 | `hot_backup.py` | 热备份数据源切换机制 |
| 统一模型 | `unified_models.py` | 共享数据模型定义 |
| 缓存策略 | `fund/cache_strategy.py` | 基金缓存策略，支持 TTL 分层 |
...
| `fund_cache_metadata` | 缓存元数据状态 |
| `commodity_cache` | 商品行情缓存 |
| `commodity_category` | 商品分类配置 |
| `news_cache` | 新闻缓存 |
| `trading_calendar` | 交易日历 |
| `exchange_holiday` | 交易所节假日 |
